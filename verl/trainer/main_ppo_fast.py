...
"""
Training script for fast thinking template.
This template removes mandatory <think> tags for faster search-and-answer.
"""

from verl import DataProto
import torch
from verl.utils.reward_score import qa_em_fast
from verl.trainer.ppo.ray_trainer import RayPPOTrainer
import re
import numpy as np

def _select_rm_score_fn(data_source):
    # Fast thinking supports the same datasets as regular search
    if data_source in ['nq', 'triviaqa', 'popqa', 'web_questions', 
                       'hotpotqa', '2wikimultihopqa', 'musique', 
                       'bamboogle', 'strategyqa']:
        return qa_em_fast.compute_score_em
    else:
        raise NotImplementedError(f"Data source {data_source} not supported")


class RewardManager():
    """The reward manager for fast thinking template."""

    def __init__(self, tokenizer, num_examine, 
                 structure_format_score=0., final_format_score=0., 
                 retrieval_score=0., format_score=0.) -> None:
    ...
    def __call__(self, data: DataProto):
        """Compute rewards for fast thinking format."""
